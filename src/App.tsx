  ...
  return <Group key={route} className="sp-root" focusTarget={focusTarget} onBack={route !== 'main' ? goBack : undefined}>
    ...
    <main className="sp-content">
      ...
      {snapshot && <>
        ...
        : route === 'main' ? <>
          {!snapshot.player.running && snapshot.player.installed && snapshot.player.supported && snapshot.player.hasKey && <>
            <Action label="Start player on this handheld" className="sp-primary" disabled={busy} onClick={() => void run(() => client.player('start'))}>Start player on this handheld</Action>
            <p className="sp-hint">The local player stays off until you start it. Starting it selects this handheld in Spotify.</p>
          </>}
...
function Settings({ snapshot, client, run, busy, onDisconnect, setAudioMode }: { snapshot: Snapshot; client: Client; run: Run; busy: boolean; onDisconnect(): void; setAudioMode(mode: AudioState['mode']): void }) {
  ...
  return <div className="sp-settings">
    ...
    <section><h3>Spotify account</h3>{snapshot.connected && <><div className="sp-status">Connected as {snapshot.profile?.name ?? 'Spotify listener'}</div><Action label={confirmSignOut ? 'Confirm sign out' : 'Sign out'} className="sp-secondary" disabled={busy} onClick={() => confirmSignOut ? void run(() => client.disconnect(), onDisconnect) : setConfirmSignOut(true)}>{confirmSignOut ? 'Confirm sign out' : 'Sign out'}</Action></>}
    ...
    </section><div className="sp-divider"/><section><h3>Listen on this handheld</h3><p>Start the local player only when you want to listen here. It stays off after a console restart or Decky reload. Once started, music keeps playing when you close QAM or launch a game.</p>
      <Group flow="row" className="sp-filters">
        <Action label="Use Soloist player" className="sp-chip" selected={snapshot.player.engine !== 'flatpak'} disabled={busy} onClick={() => void run(() => client.player('engine', 'soloist'))}>Soloist</Action>
        <Action label="Use Spotify Flatpak player" className="sp-chip" selected={snapshot.player.engine === 'flatpak'} disabled={busy} onClick={() => void run(() => client.player('engine', 'flatpak'))}>Spotify (Flatpak)</Action>
      </Group>
      <p>Soloist is the default and recommended player: responsive controls, much lower memory use and lower power consumption. Spotify (Flatpak) is an optional alternative.</p>
      {!snapshot.player.supported ? <p>This player requires a supported Linux handheld.</p> : snapshot.player.engine === 'flatpak' ? <>
        <p>Uses the Spotify app installed on this handheld. No Soloist key is needed.</p>
        {!snapshot.player.installed ? <><p>Install Spotify from Discover in Desktop mode.</p><Action label="Check Spotify installation" className="sp-secondary" disabled={busy} onClick={() => void run(() => client.player('start'))}>Check installation</Action></> : <>
          <Action label="Open Spotify app" className="sp-primary" disabled={busy} onClick={() => void run(() => client.player('open'))}>Open Spotify</Action>
          <Action label={snapshot.player.running ? 'Stop local player' : 'Start local player'} className="sp-secondary" disabled={busy} onClick={() => void run(() => client.player(snapshot.player.running ? 'stop' : 'start'))}>{snapshot.player.running ? 'Stop Spotify' : 'Start Spotify in background'}</Action>
          {!snapshot.player.linked && <p>Sign in with the same Spotify account, choose this computer in Spotify, and play a song once to link its playback device.</p>}
          {snapshot.player.linked && <p className="sp-status">Spotify playback device linked</p>}
          <p>Manage downloads and audio quality in the Spotify app. Downloaded music can play offline; search, library and queue features in SpotiDeck need internet.</p>
          <p>Closing QAM keeps Spotify running. Stop Spotify or switch to Soloist to close the app.</p>
        </>}
      </> : <>
      ...
      {snapshot.player.running && <p>Select “SpotiDeck” in the Spotify app once to pair. Later, use Start player to connect this handheld again.</p>}
    ...
    <p className="sp-note">SpotiDeck · 1.1.1<br/>Independent plugin. Not affiliated with Spotify.</p></div>;
}
