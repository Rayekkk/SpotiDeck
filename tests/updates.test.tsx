...
afterEach(cleanup);
const available: UpdateCheck = { success: true, current_version: '1.1.0', latest_version: '1.1.1', update_available: true, download_available: true, size: 2097152 };
function deferred<T>() {
...
it('downloads the selected version once and retains progress and the result across page remounts', async () => {
  ...
  await click('Check for plugin updates');
  await click('Download plugin 1.1.1');
  await click('Download plugin 1.1.1');
  expect(client.updatesDownload).toHaveBeenCalledExactlyOnceWith('1.1.1');
  page.unmount();
  ...
  await act(async () => download.resolve({ success: true, version: '1.1.1', path: '/home/deck/Downloads/SpotiDeck-1.1.1.zip' }));
  expect(screen.getByText('/home/deck/Downloads/SpotiDeck-1.1.1.zip')).toBeTruthy();
  expect(screen.getByText(/Install the ZIP through Decky/)).toBeTruthy();
  page.unmount();
  render(<UpdateSection client={client}/>);
  expect(screen.getByText('Version 1.1.1 downloaded.')).toBeTruthy();
});

it('does not offer a download for an equal, older or unverifiable release', async () => {
  const client = previewClient();
  client.updatesCheck = vi.fn()
    .mockResolvedValueOnce({ ...available, latest_version: '1.1.0', update_available: false, download_available: false })
  ...
  render(<UpdateSection client={client}/>);
  await click('Check for plugin updates');
  expect(screen.getByText('Up to date. Version 1.1.0.')).toBeTruthy();
  await click('Check for plugin updates');
...
it('shows failed checks and allows retry without duplicate concurrent requests', async () => {
  ...
  expect(screen.getByRole('button', { name: 'Download plugin 1.1.1' })).toBeTruthy();
});
...
it('ignores completion from an unloaded plugin session', async () => {
  ...
  expect(screen.queryByRole('button', { name: /Download plugin/ })).toBeNull();
  expect(screen.getByText(/Installed version: 1.1.0. Check GitHub/)).toBeTruthy();
});
